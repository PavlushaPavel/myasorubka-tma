  ...
  return (
    <div className="screen">
      ...
      <Reveal delay={0.1}>
        <div
          className="panel"
          style={{ marginTop: 14, borderColor: 'rgba(211,161,74,0.35)', background: 'rgba(11,16,20,0.6)' }}
        >
      ...
      <button className="btn btn-primary" style={{ marginTop: 18, width: '100%' }} onClick={copy}>
        {copied ? 'Скопировано!' : BABLO.cta}
      </button>
      ...
      <button
        ...
        onClick={() =>
          window.Telegram?.WebApp?.showAlert?.('Поделись ссылкой со спецом, которого тоже делали крайним')
        }
      >
        {BABLO.share}
      </button>
      ...
      <button
        className="btn btn-ghost"
        style={{ marginTop: 10, width: '100%', color: 'var(--text-faint)' }}
        onClick={() => navigateScreen(0)}
      >
        ↺ Пройти заново
      </button>
    </div>

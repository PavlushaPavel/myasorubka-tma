...
/**
 * Navigate to a screen with a cinematic View Transition.
 * - 'circle' (default): clip-path circle reveal from the last tap point
 * - 'curtain': black top→bottom wipe used before the defeat screen
...
export function navigateScreen(n: number, variant: Variant = 'circle') {
  const goToScreen = useAppStore.getState().goToScreen
